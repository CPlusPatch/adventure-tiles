...
class Tile:
    def __init__(self, type: TileType):
        self.type = type
        self.surfaces = [pygame.Surface((16, 16)) for _ in type.images]
    ...
    def load_surfaces(self, surrounding_tiles: list[TileType | None]):
        ...
        if (self.type.size == Pos(1, 1)):
            #try:
            self.surfaces[0] = pygame.image.load(self.type.get_sprite(surrounding_tiles)).convert_alpha()
        ...
        if self.type.size == Pos(1, 1):
            ...
            for x in range(self.type.size.x):
                for y in range(self.type.size.y):
                    surface.blit(self.surfaces[x + y * self.type.size.x], (x * 16, y * 16))
            return surface